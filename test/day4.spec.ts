...
describe("day 4", () => {
  ...
  const real: Problem = {
    drawn: [26,38,2,15,36,8,12,46,88,72,32,35,64,19,5,66,20,52,74,3,59,94,45,56,0,6,67,24,97,50,92,93,84,65,71,90,96,21,87,75,58,82,14,53,95,27,49,69,16,89,37,13,1,81,60,79,51,18,48,33,42,63,39,34,62,55,47,54,23,83,77,9,70,68,85,86,91,41,4,61,78,31,22,76,40,17,30,98,44,25,80,73,11,28,7,99,29,57,43,10],
    boards: [
      [
        [57, 12, 60, 96, 93],
        [73, 87, 63, 70, 91],
  ...
  describe("part 1", () => {
    [
      ...
      [example.boards[1], [0, 13, 7, 10, 16], true],
      [example.boards[1], [0, 13, 7, 10], false],
    ].forEach((v: any[]) => {
      let board: Board = v[0], drawn: DrawnNumbers, expected: boolean;
      [board, drawn, expected] = v;
      it(`knows when a board has won - ${board} [${drawn}] => ${expected}`, () => {
    ...
    it("calculates answer", () => {
      expect(finalScore(findWinner(real))).to.eql(58374)
    })
  })
